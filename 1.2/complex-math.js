...
function exp(z){
  ...
  var expra = expr(a);
  resarr[0] = multr(expra, checkE(Math.cos(Number(b))));
  resarr[1] = multr(expra, checkE(Math.sin(Number(b))));
...
function sqrtr(a, nprec){
  ...
  for (var i = 1; currprec < nprec+1; i++){ // Newton's method
    func1 = subr(powr(x[i-1], "2", nprec+2), a);
    func2 = multr("2", x[i-1]);
...
function expr(a, nprec){
  ...
  for (var i = 0; gt(absr(a), "0.5"); i++)a = subr(a, 1);
...
function expsmall(a, nprec){
  ...
  var frac = "1";
  var exp = addr(a, 1);
  for (var i = 2; roundr(frac, nprec+1) != "0"; i++){
...
function lnr(a, nprec){
  ...
  var x = [];
  x[0] = checkE(Math.log(Number(a)));
  var currprec = 0;
  var func1, func2
  for (var i = 1; currprec < nprec+1; i++){ // Newton's method
    func1 = subr(divr(a, expr(x[i-1], nprec+2), nprec+2), 1);
    x[i] = addr(func1, x[i-1]);
    currprec = getPrec(x[i-1], x[i], nprec);
  }
